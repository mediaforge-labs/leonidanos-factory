...
FACTORY_VERSION = "leonidanos-factory-v1"
...
def headers(prefer: str | None = None) -> dict[str, str]:
    out = {"apikey": SUPABASE_SERVICE_ROLE_KEY, "Content-Type": "application/json"}
...
def sb_patch(table: str, filters: dict[str, str], payload: dict[str, Any]) -> list[dict[str, Any]]:
    response = requests.patch(
        f"{SUPABASE_URL}/rest/v1/{table}", headers=headers("return=representation"),
        params=filters, json=payload, timeout=30,
    )
    response.raise_for_status()
...
def upsert_variant(payload: dict[str, Any]) -> dict[str, Any]:
    response = requests.post(
        ...
        params={"on_conflict": "queue_id,locale"}, json=payload, timeout=30,
    )
    response.raise_for_status()
    rows = response.json()
    if not rows:
...
def insert_factory_job(payload: dict[str, Any]) -> dict[str, Any] | None:
    response = requests.post(
        ...
        params={"on_conflict": "job_key"}, json=payload, timeout=30,
    )
    response.raise_for_status()
...
def select_queue(queue_id: str) -> dict[str, Any]:
    params = {
        "select": "id,blog_post_id,status,script,tts_text,youtube_title,youtube_description,thumbnail_title,thumbnail_url,youtube_privacy_status,youtube_video_id,updated_at",
        "limit": "1",
    }
    if queue_id:
        ...
        params.update({
            "status": "in.(scripted,voice_ready,failed)",
            "tts_text": "not.is.null",
...
def existing_variant(queue_id: str, locale: str) -> dict[str, Any] | None:
    rows = sb_get("youtube_video_variants", {
        "select": "*", "queue_id": f"eq.{queue_id}", "locale": f"eq.{locale}", "limit": "1",
    })
    return rows[0] if rows else None
...
def prepare_pt(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if existing and existing.get("youtube_video_id"):
        raise RuntimeError(f"PT-BR variant is already checkpointed on YouTube ({existing['youtube_video_id']}); refusing rerender")
    return upsert_variant({
        "queue_id": row["id"], "blog_post_id": row["blog_post_id"], "locale": "pt-BR",
        "channel_key": "leonidanos_pt", "status": "thumbnail_ready", "script": row.get("script"),
        "tts_text": row["source_text"], "youtube_title": row.get("youtube_title"),
        "youtube_description": row.get("youtube_description"), "thumbnail_title": row.get("thumbnail_title"),
        "thumbnail_url": row.get("thumbnail_url"), "youtube_privacy_status": "private",
...
def prepare_en(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if current and current.get("youtube_video_id"):
        raise RuntimeError(f"EN-US variant is already checkpointed on YouTube ({current['youtube_video_id']}); refusing rerender")
    ...
    if existing_text and existing_title and existing_description and existing_thumb_title:
        ...
        metadata = {"youtube_title": existing_title, "youtube_description": existing_description, "thumbnail_title": existing_thumb_title}
    ...
    payload: dict[str, Any] = {
        "queue_id": row["id"], "blog_post_id": row["blog_post_id"], "locale": "en-US",
        "channel_key": "leonidanos_en", "status": "thumbnail_ready" if (current or {}).get("thumbnail_url") else "script_ready",
        "script": english_text, "tts_text": english_text, "youtube_title": metadata["youtube_title"],
        "youtube_description": metadata["youtube_description"], "thumbnail_title": metadata["thumbnail_title"],
        "youtube_privacy_status": "private", "last_error": None, "updated_at": now_iso(),
    }
...
def ensure_factory_job(row: dict[str, Any], variant: dict[str, Any], locale: str, *, force_retry_failed: bool = False) -> dict[str, Any] | None:
    job_key = f"{FACTORY_VERSION}:{row['id']}:{locale}"
    desired_metadata = {
        "variant_id": variant["id"], "shorts_requested": 5, "music_enabled": False,
        "unique_media_only": True, "prepared_by": "leonidanos-factory",
        "factory_version": FACTORY_VERSION, "private_upload_required": True,
    }
    existing_rows = sb_get("youtube_factory_jobs", {"select": "*", "job_key": f"eq.{job_key}", "limit": "1"})
    existing = existing_rows[0] if existing_rows else None
    if not existing:
        job = insert_factory_job({
            "job_key": job_key, "queue_id": row["id"], "locale": locale, "stage": "production",
            "preferred_backend": "github", "status": "pending", "metadata": desired_metadata,
        })
        if not job:
            rows = sb_get("youtube_factory_jobs", {"select": "*", "job_key": f"eq.{job_key}", "limit": "1"})
            if not rows:
                raise RuntimeError(f"Factory job insert raced but no row exists: {job_key}")
            return rows[0]
    ...
    if status in {"pending", "running", "completed"}:
        ...
        return existing
    retryable = status in {"failed", "fallback_requested"} and (attempt < MAX_AUTOMATIC_FACTORY_ATTEMPTS or force_retry_failed)
    if not retryable:
        raise RuntimeError(f"Factory job {job_key} is {status} after {attempt} attempt(s); explicit force retry required")
    metadata = dict(existing.get("metadata") or {})
    metadata.update(desired_metadata)
    metadata["requeued_at"] = now_iso()
    payload: dict[str, Any] = {
        "status": "pending", "preferred_backend": "github", "selected_backend": None,
        "lease_owner": None, "lease_expires_at": None, "fallback_reason": None, "completed_at": None,
        "last_progress_at": now_iso(), "metadata": metadata, "updated_at": now_iso(),
    }
    if force_retry_failed and attempt >= MAX_AUTOMATIC_FACTORY_ATTEMPTS:
        payload["attempt"] = 0
    updated = sb_patch("youtube_factory_jobs", {"id": f"eq.{existing['id']}", "status": f"eq.{status}"}, payload)
    if not updated:
        raise RuntimeError(f"Factory job changed concurrently while requeueing: {job_key}")
    print(f"Factory job requeued: {job_key}")
    return updated[0]
...
def main() -> None:
    ...
    try:
        ...
        if not args.queue_id.strip() and str(exc) == "No editorial queue item is ready for MediaForge preparation":
            print(json.dumps({"status": "idle", "factory_version": FACTORY_VERSION}))
            return
        raise
    pt = prepare_pt(row)
    en = prepare_en(row)
    ...
    print(json.dumps({
        "status": "mediaforge_ready", "queue_id": row["id"], "pt_variant_id": pt["id"], "en_variant_id": en["id"],
        "factory_version": FACTORY_VERSION, "music_enabled": False, "shorts_requested_per_locale": 5,
